...
const createIngredientGroup = (name: string): IngredientGroup => ({
    name,
    ingredients: Array.from({length: getRandomNumber(3, 10)}, (_, ingredientIndex) => createIngredient((ingredientIndex + 1) * 100, `Ingredient ${ingredientIndex + 1}`)),
});

const generateInstruction = (): string => Array.from({length: getRandomNumber(5, 30)}, () => 'Instruction').join(' ');

export const data: Recipe = {
	ingredientGroups: Array.from({length: getRandomNumber(2, 4)}, (_, groupIndex) => createIngredientGroup(`Group ${groupIndex + 1}`)),
